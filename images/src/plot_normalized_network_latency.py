#!/usr/bin/env python3
"""
Normalized Network Latency Analysis
Generates a grouped bar chart comparing network latency across different routing methods
...
def plot_normalized_network_latency():
    ...
    # Read CSV data
    data_file = os.path.join(input_dir, 'normalized_network_latency.csv')
    ...
    x = np.arange(len(benchmarks)) * 1.8
    width = 0.24
    ...
    ax.set_ylabel('Normalized Network Latency', fontweight='bold', fontsize=22)
    ax.set_xticks(x)
    ...
    # Save figure
    output_file = os.path.join(output_dir, 'normalized_network_latency.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight', pad_inches=0.3)
    print(f"Figure saved to: {output_file}")

    output_pdf = os.path.join(output_dir, 'normalized_network_latency.pdf')
    ...
    # Print statistics
    print("\n" + "="*80)
    print("NORMALIZED NETWORK LATENCY ANALYSIS")
    print("="*80)
    ...
    reduction = ((avg_baseline - avg_alpha) / avg_baseline) * 100
    print(f"\nAverage ALPHA Latency Reduction vs Baseline: {reduction:.1f}%")
    print("="*80)

if __name__ == '__main__':
    plot_normalized_network_latency()
