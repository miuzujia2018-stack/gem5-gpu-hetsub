#!/usr/bin/env python3
"""
Hop Count Analysis for CPU Applications
Generates a grouped bar chart showing normalized hop count for different routing methods
Lower values indicate better performance (fewer hops)
"""
...
def plot_hop_count_cpu_applications():
    """Generate hop count analysis bar chart for CPU applications"""
    ...
    # Read CSV data
    data_file = os.path.join(input_dir, 'hop_count_cpu_applications.csv')
    ...
    shortcut = df['Shortcut'].values
    ftby = df['FTBY'].values
    ftby_pg = df['FTBY_PG'].values
    adapt_noc_norl = df['Adapt-NoC-noRL'].values
    adapt_noc = df['Adapt-NoC'].values

    # Set up the bar chart
    x = np.arange(len(benchmarks)) * 2.2
    width = 0.27
    ...
    # Customize the plot
    ax.set_xlabel('CPU Applications', fontweight='bold', fontsize=22)
    ax.set_ylabel('Hop Count (Normalized)', fontweight='bold', fontsize=22)
    ax.set_xticks(x)
    ...
    # Save figure
    output_file = os.path.join(output_dir, 'hop_count_cpu_applications_comparison.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight', pad_inches=0.3)
    print(f"Figure saved to: {output_file}")

    output_pdf = os.path.join(output_dir, 'hop_count_cpu_applications_comparison.pdf')
    ...
    # Print statistics
    print("\n" + "="*100)
    print("HOP COUNT ANALYSIS - CPU APPLICATIONS")
    print("="*100)
    print(f"{'App':<10} {'Baseline':<10} {'OSCAR':<10} {'Shortcut':<10} {'FTBY':<10} {'FTBY_PG':<10} {'NoC-noRL':<10} {'Adapt-NoC':<10}")
    print("-"*100)
    for i, app in enumerate(benchmarks):
        print(f"{app:<10} {baseline[i]:<10.2f} {oscar[i]:<10.2f} {shortcut[i]:<10.2f} {ftby[i]:<10.2f} {ftby_pg[i]:<10.2f} {adapt_noc_norl[i]:<10.2f} {adapt_noc[i]:<10.2f}")
    print("="*100)

    # Find average row (last row)
    if benchmarks[-1] == 'Average':
        print(f"\nBest performing method (Average): Adapt-NoC-noRL with {adapt_noc_norl[-1]:.2f} normalized hop count")
        reduction = ((baseline[-1] - adapt_noc_norl[-1]) / baseline[-1]) * 100
        print(f"Hop count reduction vs Baseline: {reduction:.1f}%")
    print("="*100)

if __name__ == '__main__':
    plot_hop_count_cpu_applications()
