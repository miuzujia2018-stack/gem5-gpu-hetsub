#!/usr/bin/env python3
"""
Hop Count Analysis Visualization for CPU Applications
...
def plot_hop_count_cpu():
    """Generate hop count analysis bar chart from CSV data"""
    ...
    # Read CSV data
    data_file = os.path.join(input_dir, 'hop_count_cpu_data.csv')
    ...
    ax.set_ylabel('Hop Count of CPU Applications (Normalized)', fontweight='bold', fontsize=22)
    ax.set_xticks(x)
    ...
    # Save figure
    output_file = os.path.join(output_dir, 'hop_count_cpu_applications.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight', pad_inches=0.3)
    print(f"Figure saved to: {output_file}")

    # Also save as PDF for publication quality
    output_pdf = os.path.join(output_dir, 'hop_count_cpu_applications.pdf')
    ...
    print("\n" + "="*60)
    print("HOP COUNT ANALYSIS SUMMARY (CPU Applications)")
    print("="*60)
