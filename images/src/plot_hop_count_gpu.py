#!/usr/bin/env python3
"""
Hop Count Analysis Visualization for GPU Applications
...
def plot_hop_count_gpu():
    """Generate hop count analysis bar chart for GPU applications from CSV data"""
    ...
    # Read CSV data
    data_file = os.path.join(input_dir, 'hop_count_gpu_data.csv')
    ...
    x = np.arange(len(applications)) * 1.3  # Label locations with increased spacing
    ...
    # Customize the plot
    ax.set_xlabel('GPU Applications', fontweight='bold', fontsize=22)
    ax.set_ylabel('Hop Count of GPU Applications (Normalized)', fontweight='bold', fontsize=22)
    ax.set_xticks(x)
    ...
    # Save figure
    output_file = os.path.join(output_dir, 'hop_count_gpu_applications.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight', pad_inches=0.3)
    print(f"Figure saved to: {output_file}")

    # Also save as PDF for publication quality
    output_pdf = os.path.join(output_dir, 'hop_count_gpu_applications.pdf')
    ...
    print("\n" + "="*60)
    print("HOP COUNT ANALYSIS SUMMARY (GPU Applications)")
    print("="*60)
    print(f"{'Application':<15} {'Baseline':<12} {'TB-TBP':<12} {'Proposed':<12}")
    print("-"*60)
    for i, app in enumerate(applications):
        print(f"{app:<15} {baseline[i]:<12.2f} {tb_tbp[i]:<12.2f} {proposed[i]:<12.2f}")
    print("="*60)
