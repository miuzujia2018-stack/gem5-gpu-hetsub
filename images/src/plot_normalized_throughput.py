...
Generates a grouped bar chart comparing throughput across different routing methods
...
def plot_normalized_throughput():
    """Generate normalized throughput comparison bar chart"""
    ...
    # Read CSV data
    data_file = os.path.join(input_dir, 'normalized_throughput.csv')
    ...
    ax.set_ylabel('Normalized Throughput', fontweight='bold', fontsize=22)
    ax.set_xticks(x)
    ...
    # Save figure
    output_file = os.path.join(output_dir, 'normalized_throughput.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight', pad_inches=0.3)
    print(f"Figure saved to: {output_file}")

    output_pdf = os.path.join(output_dir, 'normalized_throughput.pdf')
    ...
    # Print statistics
    print("\n" + "="*80)
    print("NORMALIZED THROUGHPUT ANALYSIS")
    print("="*80)
    ...
    print(f"\nAverage ALPHA Throughput Improvement over Baseline: {improvement:.1f}%")
    print("="*80)
