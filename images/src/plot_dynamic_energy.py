#!/usr/bin/env python3
"""
Dynamic Energy Analysis Visualization for Mixed Workload
Generates a grouped bar chart showing normalized dynamic energy for different routing methods
"""
...
def plot_dynamic_energy():
    """Generate dynamic energy analysis bar chart from CSV data"""
    ...
    # Read CSV data
    data_file = os.path.join(input_dir, 'dynamic_energy_data.csv')
    ...
    ax.set_ylabel('Dynamic Energy (Normalized)', fontweight='bold', fontsize=22)
    ax.set_xticks(x)
    ...
    # Save figure
    output_file = os.path.join(output_dir, 'dynamic_energy_mixed_workload.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight', pad_inches=0.3)
    print(f"Figure saved to: {output_file}")

    # Also save as PDF for publication quality
    output_pdf = os.path.join(output_dir, 'dynamic_energy_mixed_workload.pdf')
    ...
    print("\n" + "="*60)
    print("DYNAMIC ENERGY ANALYSIS SUMMARY")
    print("="*60)
