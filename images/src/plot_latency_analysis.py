#!/usr/bin/env python3
"""
Latency Analysis Visualization for Mixed Workload
Generates a grouped bar chart showing normalized packet latency for different routing methods
"""
...
def plot_latency_analysis():
    """Generate latency analysis bar chart from CSV data"""
    ...
    # Read CSV data
    data_file = os.path.join(input_dir, 'latency_data.csv')
    ...
    ax.set_ylabel('Packet Latency (Normalized)', fontweight='bold', fontsize=22)
    ax.set_xticks(x)
    ...
    # Save figure
    output_file = os.path.join(output_dir, 'latency_analysis_mixed_workload.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight', pad_inches=0.3)
    print(f"Figure saved to: {output_file}")

    # Also save as PDF for publication quality
    output_pdf = os.path.join(output_dir, 'latency_analysis_mixed_workload.pdf')
